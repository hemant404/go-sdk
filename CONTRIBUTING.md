# Contributing to the v12 Go SDK
...
`demo/` is a separate Go module, so the commands above do not reach it.
Build and test it on its own — and do run it, since its tests cover the demo's
...
The demo runs against a real tenant; see its README for the credentials it
needs. Never commit a filled `.env`.
